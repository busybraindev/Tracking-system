...
const Ec = ({ employee, onDelete, onEdit }) => {
  const hd = async () => {
    if (!confirm("Are You sure ou want to delete this employee?")) return;
  };
  return (
      ...
      {!employee.isDeleted && (
        <div className="absolute inset-0 bg-linear-to-t from-indigo-700/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6 gap-3">
          <button
            ...
            className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl shadow-lg transition-all hover:scale-106"
          >
