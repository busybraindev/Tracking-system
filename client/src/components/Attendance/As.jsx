...
const As = ({ history }) => {
  const tp = history.filter(
    (h) => h.status === "PRESENT" || h.status === "LATE",
  ).length;
  const tl = history.filter((h) => h.status === "LATE").length;
  const sts = [
    { label: "Days Present", value: tp, icon: CalendarIcon },
    { label: "Late Arrivals", value: tl, icon: AlertCircleIcon },
